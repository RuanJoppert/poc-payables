...
BEGIN
  ...
  EXECUTE 'SELECT
    ...
    payment_date,
    transaction_id,
    (round((amount - fee) * $6 * ((payment_date AT TIME ZONE ''America/Sao_Paulo'')::date - ($3 AT TIME ZONE ''America/Sao_Paulo'')::date) / (30 * 100)))::integer AS anticipation_fee
  ...
  WHERE
        ...
        payable.payment_date AT TIME ZONE ''America/Sao_Paulo'' > $3 AND
        ( $4 IS NULL OR payable.created_at AT TIME ZONE ''America/Sao_Paulo'' <= $4 )
    ORDER BY
