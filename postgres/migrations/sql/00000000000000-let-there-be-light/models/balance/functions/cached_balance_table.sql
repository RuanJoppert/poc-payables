...
BEGIN
  FOR selected_balance IN (
    ...
    WHERE
      ...
      ( recipient_id = query_recipient_id OR query_recipient_id = '_all' )
      FOR UPDATE
  ) LOOP
    ...
    WHERE
      ...
      selected_balance.recipient_id = bo.recipient_id                 AND
      selected_balance.status       = bo_status_to_varchar(bo.status) AND
      selected_balance.last_id      < bo.id;

    UPDATE "Balances"
    SET (last_id, amount, fee) = (operations.max_id, selected_balance.amount + operations.amount, selected_balance.fee + operations.fee)
    WHERE id = selected_balance.id;
