...
BEGIN
  ...
  IF DATE_TRUNC(query_date_type, NOW() AT TIME ZONE 'America/Sao_Paulo') < query_end_date THEN
    query_end_date = DATE_TRUNC(query_date_type, NOW() AT TIME ZONE 'America/Sao_Paulo');
  END IF;
  ...
  FOR recipient IN (
    ...
    FOR unnested_status IN (
      ...
      WHERE
        ...
        bh.histogram_date <= DATE_TRUNC(query_date_type, query_end_date AT TIME ZONE 'America/Sao_Paulo')
      ORDER BY
      ...
      IF NOT FOUND THEN
        total_amount      = 0;
        total_fee         = 0;
        min_date          = DATE_TRUNC(query_date_type, recipient.created_at AT TIME ZONE 'America/Sao_Paulo');
        need_cache_update = TRUE;
      ELSE
        IF histogram.histogram_date = DATE_TRUNC(query_date_type, query_end_date AT TIME ZONE 'America/Sao_Paulo') OR
           DATE_TRUNC(query_date_type, histogram.histogram_date      AT TIME ZONE 'America/Sao_Paulo') = 
           DATE_TRUNC(query_date_type, ( NOW() - '1 day'::interval ) AT TIME ZONE 'America/Sao_Paulo') THEN
      ...
      IF need_cache_update THEN
        FOR aggregated IN (
          SELECT
            DATE_TRUNC(query_date_type, created_at AT TIME ZONE 'America/Sao_Paulo') AS histogram_date,
          ...
          WHERE
            ...
            DATE_TRUNC(query_date_type, created_at AT TIME ZONE 'America/Sao_Paulo') > min_date
          GROUP BY
            DATE_TRUNC(query_date_type, created_at AT TIME ZONE 'America/Sao_Paulo')
          ORDER BY
            DATE_TRUNC(query_date_type, created_at AT TIME ZONE 'America/Sao_Paulo')
        ) LOOP
          ...
          IF DATE_TRUNC(query_date_type, aggregated.histogram_date AT TIME ZONE 'America/Sao_Paulo') < DATE_TRUNC(query_date_type, NOW() AT TIME ZONE 'America/Sao_Paulo') THEN
      ...
      IF query_end_date >= DATE_TRUNC(query_date_type, NOW() AT TIME ZONE 'America/Sao_Paulo') THEN
        ...
        IF last_histogram.id IS NULL THEN
          ...
          WHERE
            ...
            bh.status          = selected_status AND
            bh.date_type       = query_date_type AND
            bh.histogram_date <= DATE_TRUNC(query_date_type, query_start_date AT TIME ZONE 'America/Sao_Paulo')
          ORDER BY
            bh.histogram_date DESC
          LIMIT 1;
        ...
        WHERE
          ...
          bo.recipient_id                 = recipient.id         AND
          bo_status_to_varchar(bo.status) = selected_status      AND
          DATE_TRUNC(query_date_type, created_at AT TIME ZONE 'America/Sao_Paulo') = query_end_date
        GROUP BY
          DATE_TRUNC(query_date_type, created_at AT TIME ZONE 'America/Sao_Paulo');
