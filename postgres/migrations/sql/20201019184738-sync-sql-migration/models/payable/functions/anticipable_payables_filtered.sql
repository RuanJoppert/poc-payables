...
DECLARE
  ...
  _amount_to_anticipate           bigint;
  _gross_amount_to_anticipate     bigint;
  _payable                        anticipatable_payable;
  _anticipable_payable            anticipable_payable;
BEGIN
  _amount_to_anticipate       = 0;
  _gross_amount_to_anticipate = 0;

  IF max_gross_amount <= 0 THEN
    _max_gross_amount_to_anticipate = max_gross_amount_to_anticipate(max_volume_percentage, anticipable_company_id, company_max_antecipable_rate, anticipable_recipient_id, recipient_max_antecipable_rate, new_payment_date);
  ELSE
    _max_gross_amount_to_anticipate = max_gross_amount;
  END IF;
  ...
  FOR _payable IN (
    SELECT * FROM get_anticipatable_payables(payable_ids, anticipable_recipient_id, new_payment_date, max_created_at, anticipation_fee_rate, order_by)
  ) LOOP
